...
# 비디오 파일을 이용한 실행
1. 비디오를 이용한 실행
python detect_and_track_fish_SOLID.py --weights halibut_ver2.pt --source data/data/Infrared/feed_summery_20sec.mp4

2. 기준치 설정을 위하여 실시간 영상 및 그래프 표시를 실행
python detect_and_track_fish_SOLID.py --weights halibut_ver2.pt --source data/data/Infrared/feed_summery_20sec.mp4 --view_img --view_graph

# 웹캠을 이용한 실행
1. 웹캠을 이용한 실행
python detect_and_track_fish_SOLID.py --weights halibut_ver2.pt --source 0 --use_live_camera

2. 기준치 설정을 위하여 웹캠 영상을 입력하여 실시간 영상 및 그래프 표시를 실행
python detect_and_track_fish_SOLID.py --weights halibut_ver2.pt --source 0 --use_live_camera --view_img --view_graph
"""
...
class VideoVisualizer(IVisualizer):
    ...
    def draw_boxes(img, bbox, identities=None, categories=None, confidences=None, names=None, colors=None, opt=None):
        ...
        for i, box in enumerate(bbox):
            ...
            if not opt.nolabel:
                ...
                label = f'{id} {names[cat]}'  # ID를 포함하지 않으면 더 간단한 라벨
                tf = max(tl - 1, 1)  # font thickness
                t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
                c2 = x1 + t_size[0], y1 - t_size[1] - 3
                cv2.rectangle(img, (x1, y1), c2, color, -1, cv2.LINE_AA)  # filled
                cv2.putText(img, label, (x1, y1 - 2), cv2.FONT_HERSHEY_SIMPLEX, tl / 3, (255, 255, 255), thickness=tf, lineType=cv2.LINE_AA)
        return img
    ...
    def visualize(self):
        ...
        if self.dataset.mode != 'image' and self.opt.show_fps:
            ...
            cv2.putText(self.im0, f"FPS: {int(fps)}", (20, 70), cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), 2)
...
class VideoProcessor:
    ...
    def run(self):
        ...
        for path, img, im0s, vid_cap in self.dataset:
            ...
            if self.opt.view_graph and (current_time is not None):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exist_ok', action='store_true', help='existing project/name ok')
    ...
    parser.add_argument('--unique_track_color', action='store_true', help='unique track color')
    ...
    parser.add_argument('--save_img', action='store_true', help='save images/videos')
    parser.add_argument('--view_img', action='store_true', help='display results')
    parser.add_argument('--view_graph', action='store_true', help='show video')
