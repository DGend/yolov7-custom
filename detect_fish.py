...
class Detector:
    def __init__(self, opt):
        self.opt = opt
        self.save_img = not opt.nosave and not opt.source.endswith('.txt')  # save inference images
        self.webcam = opt.source.isnumeric() or opt.source.endswith('.txt') or opt.source.lower().startswith(
    ...
    def setup_save_dir(self):
        save_dir = Path(increment_path(Path(self.opt.project) / self.opt.name, exist_ok=self.opt.exist_ok))
        if not self.opt.nosave:
            (save_dir / 'labels' if self.opt.save_txt else save_dir).mkdir(parents=True, exist_ok=True)
        return save_dir
...
############################################ detect() ############################################
    ...
    def preprocess_input(self, img, device, half_precision):
        """입력 이미지 전처리"""
    ...
    def warmup(self, img, old_img_b, old_img_h, old_img_w):
        ...
        return old_img_b, old_img_h, old_img_w
    ...
    def run(self):
        ...
        for path, img, im0s, vid_cap in dataset:
            self.img = self.preprocess_input(img, self.device, self.opt.half)
            
            old_img_b, old_img_h, old_img_w = self.warmup(self.img, old_img_b, old_img_h, old_img_w)
            
        # Inference
        t1 = time_synchronized()
        pred = self.model(self.img, augment=opt.augment)[0]
        ...
        self.process_detections(pred, im0s, path, dataset, vid_writer, vid_cap)
    
    
    def process_detections(self, pred, im0s, path, dataset, vid_writer, vid_cap):
        for i, det in enumerate(pred):  # detections per image
                ...
                if len(tracked_dets)>0:
                    ...
                    if opt.show_track:
                        ...
                        for t, track in enumerate(tracks):
                            ...
                            track_color = self.colors[int(track.detclass)] if not opt.unique_track_color else sort_tracker.color_list[t]
            ...
            im0 = self.draw_boxes(im0, bbox_xyxy, identities, categories, confidences, self.names, self.colors)
